...
namespace PathTracer
{
  ...
  {
        public static Vector3 PhongBlinnSample(double alfa)
        {
            ...
            double thetaH = Math.Acos(Math.Pow(u, 1/(alfa+1)));
            double theta = 2 * thetaH;

            return Utils.SphericalDirection(Math.Sin(theta), Math.Cos(theta), phi);
        }
